...
function renderTree(data) {
  ...
  $("#jstree").on("select_node.jstree", function (e, data) {
    ...
    // Limpiar exclusiones manuales si el nodo se selecciona de nuevo en el árbol
    if (node) clearCadenaExclusionsForNode(node);
...
function excludeCadenaRowsForTreeNode(tree, node) {
  ...
  rows.forEach(function (row) {
    var rowKey = cadenaRowKey(row);
    if (!rowKey) return;
    // La exclusión se guarda por conteo para permitir borrar una sola ocurrencia
    // aunque existan filas idénticas dentro del mismo nodo.
...
function renderCadenaPreviewTable(entries) {
  ...
  html += '<th rowspan="2" class="cadena-th-action" title="Eliminar todos los elementos seleccionados"><button type="button" class="cadena-row-remove cadena-row-remove-all" onclick="clearAllTreeSelection()" aria-label="Eliminar todos los elementos seleccionados">✕</button></th>';
  html += "</tr>";
  ...
  entries.forEach(function (entry) {
    ...
    html += '<button type="button" class="cadena-row-remove" data-key="' + esc(encodeURIComponent(key)) + '" title="Eliminar fila uno por uno">✕</button>';
    html += '</td>';
    html += "</tr>";
  });
...
function removeCadenaPreviewRow(key) {
  ...
  var selectedIdsBefore = Object.keys(treeSelectedNodes || {});
  var currentEntries = typeof buildCadenaPreviewEntriesOptimized === 'function'
    ? buildCadenaPreviewEntriesOptimized(selectedIdsBefore)
    : [];
  var matchedEntry = null;
  for (var i = 0; i < currentEntries.length; i++) {
    if (currentEntries[i] && currentEntries[i].key === key) {
      matchedEntry = currentEntries[i];
      break;
    }
  }

  // Excluir solo una ocurrencia de esta fila específica (nodeId||rowKey)
...
function removeCadenaPreviewRowOnly(key) {
  ...
  // Marcar solo la combinación específica nodeId||rowKey para ocultarla
  incrementCadenaExcludedRowCount(key);
...
function collectCadenaRowsFromTreeNode(node, nodeId, rows) {
  ...
  if (!node || !nodeId) return rows;
    ...
    var allowChildExpansion = nodeTag === "BDA_CONTENT" || nodeTag === "DA_TYPE_CONTENT" ||
                              nodeTag === "DO" || nodeTag === "DO_SYNTH" || nodeTag === "DO_TYPE_CONTENT";
  ...
  childrenToProcess.forEach(function (childId) {
    ...
    // Para DO/DO_SYNTH/DO_TYPE_CONTENT, solo incluir DA, SDO, DA_SYNTH, SDO_SYNTH hijos directos
    if ((nodeTag === "DO" || nodeTag === "DO_SYNTH" || nodeTag === "DO_TYPE_CONTENT") &&
        ["DA", "SDO", "DA_SYNTH", "SDO_SYNTH"].indexOf(childTag) === -1) {
      return;
    }
