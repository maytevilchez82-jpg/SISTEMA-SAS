...
function renderDetail(meta) {
  ...
  if (tag === "LN" || tag === "LN0" || tag === "LN_SYNTH") {
    ...
    html +=
      section("Data Object", [
        ["name", doNameValue, "highlight"],
        ["DOType ID", doTypeId, "blue"],
        ["CDC", cdc || "", "pink"],
      ]);
    if (dtd) {
      html += '<div class="detail-section">';
      ...
      html += "</div>";
      html += extraSection("Detalles adicionales", [
        ["DOType", doTypeId, "blue"],
        ["CDC", cdc || "", "pink"],
        ["Atributos DA", String(dtd.das.length || 0) + " elemento(s)", ""],
      ]);
    }
...
function buildTreeNode(xmlNode, depth, context) {
  ...
  var ctx = JSON.parse(JSON.stringify(context || {}));
  ...
  if (tag === "DAType" && geta("id") && daTypeIndex[geta("id")]) {
    ...
    children.push({
      ...
      _meta: { tag: "DA_TYPE_CONTENT", attrs: { id: geta("id") }, ctx: JSON.parse(JSON.stringify(ctx || {})) },
    });
  }
  ...
  if (tag === "FCDA") {
    ...
    var concatValue = lnIdentifier + (doName ? "." + doName : "") + (daName ? "." + daName : "");
    lnRecords.push({
      IED: ctx.iedName || "",
      DataSet: ctx.dataSet || "",
      LDInst: resolvedLdInst,
...
function buildSdoContenidoNode(sdo, ctx) {
  ...
  return {
    ...
    attributes: { tag: "SDO_SYNTH", attrs: { name: sdoName, type: sdoTypeId } },
    _meta: {
      tag: "SDO_SYNTH",
      attrs: { name: sdoName, type: sdoTypeId },
      ctx: JSON.parse(JSON.stringify(ctx || {})),
    },
  };
}
...
function buildBdaContenidoNode(bda, ctx) {
  ...
  return {
    ...
    _meta: { tag: "BDA_CONTENT", attrs: bdaAttrs, ctx: ctx ? JSON.parse(JSON.stringify(ctx)) : {} },
  };
}
...
function exportDatasetStyledFallback() {
  ...
  exportWithSheetJS("CID_Dataset.xlsx", rows, "Dataset");
...
function renderTree(data) {
  ...
  function openNodeDescendants(tree, node) {
    ...
    node.children.forEach(function (childId) {
      ...
      openNodeDescendants(tree, childNode);
    });
  }

  function selectNodeDescendants(tree, node) {
    if (!tree || !node || !Array.isArray(node.children_d) || !node.children_d.length) return;

    var skipTags = {
      DO_TYPE_CONTENT: true,
      DA_TYPE_CONTENT: true,
      BDA_CONTENT: true,
      DO_TYPE_GROUP: true,
      LNTYPE_GROUP: true,
      ENUMTYPE_GROUP: true,
    ...
    node.children_d.forEach(function (childId) {
      var childNode = tree.get_node(childId);
  ...
  $("#jstree").on("select_node.jstree", function (e, data) {
    ...
    if (node && node.original && node.original._meta) {
      ...
      if (jstreeInstance && node && (nodeTag === "DO" || nodeTag === "DO_SYNTH")) {
        try {
          jstreeInstance.check_node(node);
        } catch (e) {}
        if (node.children && node.children.length) {
          jstreeInstance.open_node(node);
          openNodeDescendants(jstreeInstance, node);
          selectNodeDescendants(jstreeInstance, node);
        }
      }
      ...
      if (nodeTagUpper === "LN_SYNTH" || nodeTagUpper === "LNODETYPE" || nodeTagUpper === "LN" || nodeTagUpper === "LN0") {
        ...
        if (tree2) {
          // Abrir el nodo para cargar sus hijos
          ...
          // Recorrer todos los descendientes buscando DO_SYNTH / DO
          var allDescendants = node.children_d || [];
          allDescendants.forEach(function(childId) {
            var childNode = tree2.get_node(childId);
            if (!childNode || !childNode.original || !childNode.original._meta) return;
            var childTag = String(childNode.original._meta.tag || "").toUpperCase();
            if (childTag !== "DO_SYNTH" && childTag !== "DO") return;
            var childParentIds = Array.isArray(childNode.parents) ? childNode.parents.filter(function(pid){ return pid && pid !== "#"; }) : [];
            var childParentPath = childParentIds.map(function(pid){
              var pn = tree2.get_node(pid);
              return pn && pn.text ? pn.text : "";
            }).filter(Boolean).concat([childNode.text]).join(" / ");
            var childAttrs = Object.assign({}, childNode.original._meta.attrs || {});
            var childCtx = Object.assign({}, childNode.original._meta.ctx || {});
            tree2.check_node(childNode);
            treeSelectedNodes[childNode.id] = enrichStoredTreeNode({
              text: childNode.text,
              tag: childTag,
              attrs: childAttrs,
          ...
          updateTreeSelectionDisplay();
          switchTab("excel");
          setStatus("Se seleccionaron todos los DO del nodo.");
        }
        return;
      }
  ...
  $("#jstree").on("check_node.jstree", function () {
    syncTreeSelection();
  });
  ...
  var CONTENIDO_TAGS_SET = {
    ...
    "FCDA_GROUP": true,
    "EnumType": true, "ENUMTYPE": true, "DOType": true, "DAType": true, "DA_TYPE_CONTENT": true
    ...
    setTimeout(function () {
      ...
      allNodes.forEach(function (node) {
        ...
        if (CONTENIDO_TAGS_SET[tag] || tag === "ENUMTYPE") {
          ...
          var li = document.getElementById(node.id);
          if (li) li.classList.add("no-checkbox-li");
        }
      });
    }, 0);
  }
...
function updateHeaderMetaInfo(xmlDoc) {
  var ied = getFirstNodeByLocalName(xmlDoc, "IED");
  var header = getFirstNodeByLocalName(xmlDoc, "Header");
  var scl = xmlDoc && xmlDoc.documentElement ? xmlDoc.documentElement : null;

  var metaName = ied && ied.getAttribute ? (ied.getAttribute("name") || "—") : "—";
  var metaType = ied && ied.getAttribute ? (ied.getAttribute("type") || "") : "";
  var metaManufacturer = ied && ied.getAttribute ? (ied.getAttribute("manufacturer") || "") : "";
  var metaVersion = ied && ied.getAttribute ? (ied.getAttribute("configVersion") || "") : "";
...
function buildCadenaPreviewEntries(selectedIds) {
  ...
  (selectedIds || []).forEach(function (id) {
    ...
    rows.forEach(function (row) {
      ...
      if (seen[entryKey] || cadenaExcludedRowKeys[entryKey]) return;
      seen[entryKey] = true;
...
function excludeCadenaRowsForTreeNode(tree, node) {
  ...
  rows.forEach(function (row) {
    var rowKey = cadenaRowKey(row);
    if (!rowKey) return;
    cadenaExcludedRowKeys[rowKey] = true;
    cadenaExcludedRowKeys[node.id + "||" + rowKey] = true;
  });
}
...
function renderCadenaPreviewTable(entries) {
  ...
  html += '<th rowspan="2" class="cadena-th-action">✕</th>';
  html += "</tr>";
  ...
  entries.forEach(function (entry) {
    ...
    var key = entry.key || ((entry.nodeId || "") + "||" + cadenaRowKey(row));
    var descFields = getCadenaDescriptionFields(entry);
    var rowStyle = entry.matched ? "background: #fef3c7;" : "";
    var rowClass = entry.matched ? " class=\"cadena-row-matched\"" : "";
    html += "<tr" + rowClass + " style=\"" + rowStyle + "\">";
    html += "<td>" + esc(row[1] || "") + "</td>";
    html += "<td>" + esc(row[2] || "") + "</td>";
    html += "<td>" + esc(row[3] || "") + "</td>";
    html += "<td>" + esc(row[4] || "") + "</td>";
    html += "<td>" + esc(row[5] || "") + "</td>";
    html += "<td>" + esc(row[6] || "") + "</td>";
    ...
    html += '<td class="cadena-td-action"><button type="button" class="cadena-row-remove" data-key="' + esc(encodeURIComponent(key)) + '">✕</button></td>';
    html += "</tr>";
  });
...
function updateTreeSelectionDisplay() {
  ...
  var entries = buildCadenaPreviewEntries(selectedIds);
  ...
  if (summaryDiv) {
    summaryDiv.innerHTML =
      ...
      '<span style="background: rgba(30, 41, 59, 0.75); border: 1px solid rgba(148, 163, 184, 0.25); color: #e2e8f0; padding: 4px 8px; border-radius: 999px; font-size: 11px;">Filas vista previa: ' + entries.length + '</span>';
  }

  if (selectedIds.length === 0 || !entries.length) {
  ...
  listDiv.querySelectorAll(".cadena-row-remove").forEach(function (btn) {
    btn.addEventListener("click", function (e) {
      ...
      e.stopPropagation();
      var key = btn.getAttribute("data-key") || "";
      removeCadenaPreviewRow(decodeURIComponent(key));
    });
  });
}

function removeCadenaPreviewRow(key) {
  if (!key) return;

  // Siempre excluir esta fila específica
  cadenaExcludedRowKeys[key] = true;

  var parts = String(key).split("||");
  var nodeId = parts.length > 1 ? parts[0] : "";
  var rowKey = parts.length > 1 ? parts.slice(1).join("||") : parts[0];
  if (rowKey) cadenaExcludedRowKeys[rowKey] = true;

  // Contar cuántas filas quedan del mismo nodo después de excluir esta
  if (nodeId && treeSelectedNodes[nodeId]) {
    var node = treeSelectedNodes[nodeId];
    var tempRows = [];
    collectCadenaRowsFromTreeNode(node, nodeId, tempRows);
    var remainingRows = tempRows.filter(function(row) {
      var rk = nodeId + "||" + cadenaRowKey(row);
      return !cadenaExcludedRowKeys[rk];
    });

    // Solo eliminar el nodo del árbol si ya no quedan filas
    if (remainingRows.length === 0) {
      var tree = window._cachedJstree || (window._cachedJstree = (window.jQuery && window.jQuery("#jstree").length ? window.jQuery("#jstree").jstree(true) : null));
      if (tree && typeof tree.uncheck_node === "function") {
        try { tree.uncheck_node(nodeId); } catch (e) {}
      }
      delete treeSelectedNodes[nodeId];
      var orderIdx = treeSelectedOrder.indexOf(nodeId);
      if (orderIdx !== -1) treeSelectedOrder.splice(orderIdx, 1);
...
function expandDaToCadenaPaths(daDef) {
  ...
  var paths = [];
  var bdas = (daDef.type && daTypeIndex[daDef.type] && daTypeIndex[daDef.type].bdas) || [];
  if (!bdas.length) {
...
function collectCadenaRowsFromTreeNode(node, nodeId, rows) {
  ...
  if (!node || !nodeId) return rows;
    ...
    // DO y DA ya se expanden completamente desde su tipo; si además recorremos
...
function applyCadenaSheetStyle(sheet) {
  ...
  var headerGroups = [
    ...
    [bTypeCol + "1", "FF44546A"],
    [fcCol + "1", "FF44546A"],
    [resultCol + "1", "FFB4B4B4"],
    [ldDescCol + "1", "FF70AD47"],
  ];
  ...
  var subHeaders = [["C2", "FFED7D31"], ["D2", "FFED7D31"], ["E2", "FFED7D31"], ["F2", "FFB4B4B4"], ["G2", "FFB4B4B4"], [ldDescCol + "2", "FF70AD47"], [lnDescCol + "2", "FF70AD47"], [doDescCol + "2", "FF70AD47"]];
  for (var j = 0; j < pathCols; j++) {
    subHeaders.push([getExcelColumnName(8 + j) + "2", "FF44546A"]);
  }
